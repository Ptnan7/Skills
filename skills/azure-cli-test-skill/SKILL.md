...
9. **Reset test mode after live runs.** In a persistent PowerShell session, `azdev test --live` can leave `AZURE_TEST_MODE=Record` and `AZURE_TEST_RUN_LIVE=TRUE` set. A later command without `--live` can therefore still contact Azure and overwrite/fail recordings. Before playback validation, explicitly set `$env:AZURE_TEST_MODE='Playback'` and remove `Env:AZURE_TEST_RUN_LIVE`; inspect relevant environment variables when a supposed playback run creates random resource groups or returns live service errors.
10. **Remember that `ScenarioTest.cmd` formats command strings.** `azure-cli-testsdk` calls `command.format(**kwargs)` before execution, so literal raw JSON braces in command strings must be protected. Prefer a mixin/helper that escapes literal `{` and `}` before calling `super().cmd(...)`, or double braces in f-strings where interpolation is needed.
11. **Use generated AAZ argument shape, not REST payload shape.** For generated AAZ commands, raw JSON must match the generated CLI arg schema:
