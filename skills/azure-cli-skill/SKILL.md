...
| [swagger-diff.md](references/swagger-diff.md) | CLI-specific interpretation after the shared swagger diff |
...
5. **Diff swagger** — load and follow the `azure-edge-swagger-diff` skill. It runs the shared diff script, saves the report under `swagger-diffs/`, and produces the updated APIs, new APIs, removed APIs, breaking changes, and AddSwagger resource candidates. Wait for user acknowledgement of the saved diff before generation.
...
7. **Export + Generate CLI** — If the user answers yes (or `--auto-export` is used), `auto_select_resources.py` exports the workspace to `aaz` and generates extension CLI code. If `cdn` generation reports a missing target AAZ command model for a nested/property command, load `azure-cli-aaz-property-subcommands` and recreate the target subcommand before regenerating CLI. If the user answers no, export manually in Web UI, then run `python .github\skills\azure-cli-skill\scripts\generate_cli.py --ext <cdn|front-door> --version <ver>`. After generation, ask whether to run linter; use `--run-linter` for non-interactive yes or `--no-run-linter` to skip.
...
- For `cdn` swagger/API upgrades, if a missing target command model is a property/subresource subcommand, load `azure-cli-aaz-property-subcommands` and recreate or refresh the target subcommand before regenerating CLI.
...
- Before staging, committing, pushing, or opening a PR, explicitly check whether version/release metadata needs updates for the touched repo: `src/<ext>/HISTORY.rst`, `src/<ext>/setup.py`, extension package `__init__.py`, any repo-specific `ChangeLog.md`, `history`, `settings.py`/`setting.py`, or equivalent metadata. If no update is needed, say why. Do not let generated code and recordings be the only PR content when a version/changelog bump is expected.
- Before opening a PR, check `git status --short`, `git diff --cached --stat` or `git show --stat HEAD`, and the active GitHub CLI account. Use `gh auth status`; if an Enterprise Managed User account cannot create cross-fork PRs, switch to the fork owner account (for example `gh auth switch --hostname github.com --user <fork-owner>`) and retry. If a PR body file was deleted or reverted, use an inline body or recreate the file before calling `gh pr create`.
- Never stage or commit automatically unless the user has asked to commit. After version/changelog work, show `git status --short` in both the `extension` and `aaz` repos, then ask whether to commit.
