...
- For `cdn` swagger/API upgrades, do not blindly force every existing command to the target API. Before CLI generation, check [cdn-missing-target-command-model.md](issues/cdn-missing-target-command-model.md). If a missing target command model is a property/subresource subcommand, load `azure-cli-aaz-property-subcommands` and try to generate or refresh the target subcommand before preserving the old API.
