...
7. **Export + Generate CLI** — If the user answers yes (or `--auto-export` is used), `auto_select_resources.py` exports the workspace to `aaz` and generates extension CLI code. If the user answers no, export manually in Web UI, then run `python .github\skills\azure-cli-skill\scripts\generate_cli.py --ext <cdn|front-door> --version <ver>`. After generation, ask whether to run tests and linter; use `--run-checks` for non-interactive yes or `--no-run-checks` to skip.
